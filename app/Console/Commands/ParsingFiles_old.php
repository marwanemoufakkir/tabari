...
class ParsingFiles_old extends Command
{
/**
...
$doc = new \DOMDocument;
$files = glob("surah0-114-aug22/*/*.xml",GLOB_BRACE);
...
$chapterQuery="//tei:div[@type='chapter']";
$chapter = $xpath->query($chapterQuery);
if($chapter->length){
foreach ($chapter as $valueC) {
$query = "//tei:div[@type='section']";

$elements = $xpath->query($query,$valueC);
$lstvolPage='1:136';
$lstayah='';
...
$json = preg_replace('/(\s+)?\\\t(\s+)?/', ' ', json_encode(array("chapter"=>$valueC->getattribute("n"),"ayah" => $number, "ayahTitle" => trim($title),"content"=>trim($topic->nodeValue),"xml_content"=>$htmlString,"type"=> $type,'topic' => $list,'subtopic' => $subtopicList,'narrator'=> $pers,'vol'=>$volPage,'timestamp' => strtotime("-1d")), JSON_UNESCAPED_UNICODE));
$json = preg_replace('/(\s+)?\\\n(\s+)?/', ' ',$json);
...
$json = preg_replace('/(\s+)?\\\t(\s+)?/', ' ', json_encode(array("chapter"=>explode('_section',explode('sure_',$filename)[1])[0],"ayah" => $number, "ayahTitle" => trim($title),"content"=>trim($topic->nodeValue),"xml_content"=>$htmlString,"type"=> $type,'topic' => $list,'subtopic' => $subtopicList,'narrator'=> $pers,'vol'=>$volPage,'timestamp' => strtotime("-1d")), JSON_UNESCAPED_UNICODE));
$json = preg_replace('/(\s+)?\\\n(\s+)?/', ' ',$json);
$result=$this->elasticsearch->index([
'index' => 'my-tafsir3',
'type' => '_doc',
